...
vim.api.nvim_buf_set_keymap(
  ...
  "i",
  "<CR>",
  '<CMD>:lua require("terminal.scripts.open-file").jump()<CR>',
...
function M.jump()
  if not _current_opened_win then return end
  ...
  if is_invalid_path then return vim.notify("路径不存在，请重试", vim.log.levels.ERROR) end

  vim.api.nvim_command("edit " .. abs_path)

  local target_line, target_col = rel_path:match("[#:](%d+)[#:](%d+)$")
  if not target_line then target_line = rel_path:match("[#:](%d+)$") end
